...
# def az_generate_embeddings(text):
#     print("emb_engine", emb_engine)
#     openai.api_version = "2023-05-15"
#     response = openai.Embedding.create(input=text, engine=emb_engine)
#     embeddings = response['data'][0]['embedding']

#     return embeddings

def openai_generate_embedding(text):
    client = OpenAI()
...
# Azure Knowledgebase Search
credential = AzureKeyCredential(key)
azcs_search_client = SearchClient(service_endpoint, index_name =index_name , credential=credential)

def az_search_knowledgebase(search_query):

    # Get embeddings from openAI
    embeddings = openai_generate_embedding(search_query)

    # Get search vector from Azure search api
    vector = Vector(value= embeddings, k=3, fields="embedding")
    print("search query: ", search_query)
    # print("products: ", products.split(","))
    # product_filter = " or ".join([f"product eq '{product}'" for product in products.split(",")])
    
    # Perform vector search on products using azure search
...
def add_to_cache(search_query, gpt_response):
    search_doc = {
                 ...
                 "search_query_vector" : az_generate_embeddings(search_query),
                "gpt_response" : gpt_response
              }
    azcs_semantic_cache_search_client.upload_documents(documents = [search_doc])

def get_cache(search_query):
    vector = Vector(value=az_generate_embeddings(search_query), k=3, fields="search_query_vector")
...
class Smart_Agent(Agent):
    ...
    def run(self, user_input, conversation=None, stream = False, api_version = "2023-07-01-preview"):
        ...
        while True:
            # Step 1: Get the first conversation result
            response = openai.ChatCompletion.create(
...
Answer ONLY with the facts from the search tool. If there isn't enough information, say you don't know. 
...
AVAILABLE_FUNCTIONS = {
            "search_knowledgebase": az_search_knowledgebase,
